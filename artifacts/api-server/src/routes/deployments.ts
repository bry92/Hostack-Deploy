...
router.get("/projects/:projectId/deployments", asyncHandler(async (req, res) => {
  if (!req.isAuthenticated()) {
    res.status(401).json({ error: "Unauthorized" });
    return;
  }
...
router.post("/projects/:projectId/deployments", asyncHandler(async (req, res) => {
  if (!req.isAuthenticated()) {
    res.status(401).json({ error: "Unauthorized" });
    return;
  }
...
router.post("/deployments/:deploymentId/promote", asyncHandler(async (req, res) => {
  if (!req.isAuthenticated()) {
    res.status(401).json({ error: "Unauthorized" });
    return;
  }
...
router.post("/projects/:projectId/deployments/:deploymentId/rollback", asyncHandler(async (req, res) => {
  if (!req.isAuthenticated()) {
    res.status(401).json({ error: "Unauthorized" });
    return;
  }
...
router.get("/deployments/:deploymentId/stream", asyncHandler(async (req, res) => {
  if (!req.isAuthenticated()) {
    res.status(401).json({ error: "Unauthorized" });
    return;
  }
  ...
  try {
    ...
    console.error("Error setting up deployment stream:", error);
